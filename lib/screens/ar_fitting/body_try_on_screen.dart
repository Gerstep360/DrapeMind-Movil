...
class _DemoGarmentPainter extends CustomPainter {
  final BodyAnchors? body;
  _DemoGarmentPainter(this.body);
  @override
  void paint(Canvas canvas, Size size) {
    final b = body;
    if (b == null) return;
    Offset p(double x, double y) => b.clothPoint(x, y, size);
    final outline = Path();
    final points = [
      p(.30, -.08),
      p(-.08, -.04),
    ...
    outline.addPolygon(points, false);
    ...
    outline.close();
    canvas.drawShadow(outline, Colors.black38, 3, false);
    canvas.drawPath(outline, Paint()..color = const Color(0xFFF4F5EA));
    canvas.save();
    canvas.clipPath(outline);
    final stripe = Path()
      ..addPolygon([p(-.4, .38), p(1.4, .12), p(1.4, .34), p(-.4, .60)], true);
    canvas.drawPath(stripe, Paint()..color = AppColors.lime);
    canvas.restore();
    canvas.drawPath(
      outline,
      Paint()
    ...
    final seam = Path()
      ..moveTo(p(.03, .92).dx, p(.03, .92).dy)
      ..lineTo(p(.97, .92).dx, p(.97, .92).dy);
    canvas.drawPath(
      seam,
      Paint()
  ...
  @override
  bool shouldRepaint(_DemoGarmentPainter oldDelegate) =>
